...
@testset "KiteUtils.jl: legacy log columns" begin
    ...
    for name in ("Test_flight", "transition", "sim_log", "failure_low_right")
        log = load_log(name)
        # Single-tether logs store l_tether/v_reelout/winch_force as scalars
        # rather than one entry per winch. Materialising threw before they were
        # widened into slot 1.
        row = log.syslog[1]
        @test length(row.l_tether) == 4
        @test length(row.v_reelout) == 4
        @test length(row.winch_force) == 4
...
@testset "KiteUtils.jl: differential state" begin
    ...
    set_data_path(tempdir())
    P, O, D, L = 3, 2, 2, 4
    logger = Logger(P, O, D, L, 2; precision=Float64)
    written = map(1:2) do step
        ss = SysState{P, O, D, L, Float64}()
        ...
        ss.twist_surface_angle .= [0.1, 0.2 + 2.0^-40 * step]
        ss.twist_surface_vel .= [0.3, 0.4]
        ss.pulley_len .= [17 + 2.0^-40 * step, 18, 19, 20]
        ss.pulley_vel .= [21, 22, 23, 24]
        ss.l_tether .= [25 + 2.0^-40 * step, 26, 27, 28]
        ss.v_reelout .= [29, 30, 31, 32]
        log!(logger, ss)
        ss
    end
